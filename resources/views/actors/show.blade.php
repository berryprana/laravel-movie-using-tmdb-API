...
@section('content')
    <div class="movie-info border-b border-gray-800">
        <div class="container mx-auto px-16 py-16 flex flex-col md:flex-row">
            ...
            <div class="md:ml-24">
            <h2 class="text-4xl font-semibold mt-1">{{ $actor['name'] }}</h2>
                ...
                <h4 class="font-semibold mt-12">Known For</h4>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8">
                        @foreach ($knownForMovies as $knownfor)
                        <div class="mt-4">
                            <a href="{{ route('movies.show', $knownfor['id']) }}"><img src="{{ $knownfor['poster_path'] }}"
                                alt="poster" class="hover:opacity-75 transition ease-in-out duration-150">
                            </a>
                            <a href="{{ route('movies.show', $knownfor['id']) }}" class="text-sm leading-normal block text-gray-400 hover:text-white mt-1">
                                {{ $knownfor['title'] }}
                            </a>
                        </div>                          
                        @endforeach
